// Doori voice and video calls with multi-provider fallback: Agora (Primary) -> Daily (Fallback 1) -> GetStream (Fallback 2).
(() => {
'use strict';
...
const call={id:null,ref:null,type:'audio',peer:null,outgoing:false,scope:'direct',groupId:null,start:null,status:null,currentProvider:'agora',currentRole:'primary',failedProviders:[]};
const resetCall=()=>Object.assign(call,{id:null,ref:null,type:'audio',peer:null,outgoing:false,scope:'direct',groupId:null,start:null,status:null,currentProvider:'agora',currentRole:'primary',failedProviders:[]});
...
function showIncoming(data,id){
 ...
 Object.assign(call,{id,ref:window.db.collection('calls').doc(id),type:data.callType||'audio',peer:data.caller,outgoing:false,scope:'direct',status:'calling',failedProviders:[]});
 bindCallDoc();
 call.ref.update({status:'ringing',ringingAt:firebase.firestore.FieldValue.serverTimestamp()}).catch(()=>{});
...
async function fetchTelephonySession(scope,id,type,failedProviders=[]){
 if(!window.accountFunctions)throw new Error('Functions unavailable');
 try{
  const fn=window.accountFunctions.httpsCallable('getTelephonySession');
  const res=await fn({scope,id,type,failedProviders});
  return res.data;
 }catch(err){
  if(failedProviders.includes('agora'))throw err;
  const res=await tokenFunction()({scope,id});
  return {provider:'agora',role:'primary',failoverTimeoutMs:6000,...res.data};
 }
}

async function connectAgora(access,scope,type,onConnected,onFail){
 ...
 client.on('user-published',async(user,mediaType)=>{
  try{
   ...
   if(mediaType==='audio'){remoteAudio.set(user.uid,user.audioTrack);user.audioTrack.setVolume(soundMuted?0:100);user.audioTrack.play();}
   else if(type==='video'){remoteVideo.set(user.uid,user.videoTrack);const target=scope==='group'?ensureGroupVideoTarget(user.uid):$('video-remote');user.videoTrack.play(target);}
  }catch(e){console.warn('Agora track subscribe error',e);}
 });
 client.on('user-unpublished',(user,mediaType)=>{
  if(mediaType==='audio')remoteAudio.delete(user.uid);
  else{remoteVideo.delete(user.uid);$(`agora-video-${cssId(user.uid)}`)?.remove();}
 });
 client.on('user-left',user=>{
  remoteAudio.delete(user.uid);remoteVideo.delete(user.uid);
  $(`agora-video-${cssId(user.uid)}`)?.remove();
  if(scope==='direct'&&!ending)finish(false);
  updateGroupCount();
 });
 client.on('connection-state-change',state=>{
  if(state==='CONNECTED'){onConnected();}
  else if(state==='RECONNECTING')status('reconnecting',type);
  else if(state==='DISCONNECTED'&&!ending){status('failed',type);onFail(new Error('Agora disconnected'));}
 });
 ...
 if(type==='video'){
  localVideo=await window.AgoraRTC.createCameraVideoTrack({encoderConfig:'720p_2',facingMode:'user'});
  tracks.push(localVideo);
  localVideo.play(ui.localVideo);
 }
 ...
 try{
  ...
  if(Daily&&typeof Daily.createCallObject==='function'){
   ...
   dailyClient.on('track-started',ev=>{
    ...
    }else if(ev.track?.kind==='video'&&type==='video'){
     const v=document.createElement('video');v.autoplay=true;v.playsInline=true;v.srcObject=new MediaStream([ev.track]);
     const target=scope==='group'?ensureGroupVideoTarget(ev.participant.session_id):$('video-remote');
     if(target){target.innerHTML='';target.appendChild(v);}
     remoteVideo.set(ev.participant.session_id,{stop:()=>{ev.track.stop();v.srcObject=null;}});
    }
   });
...
async function connectWebRtcFallback(provider,access,scope,type,onConnected,onFail){
 try{
  ...
  if(type==='video'&&ui.localVideo){
   ui.localVideo.innerHTML='';
   const v=document.createElement('video');v.autoplay=true;v.playsInline=true;v.muted=true;v.srcObject=localMediaStream;ui.localVideo.appendChild(v);
  }
  ...
  pc.ontrack=ev=>{
   ...
   if(ev.track.kind==='audio'){
    const audio=new Audio();audio.srcObject=stream;audio.play().catch(()=>{});
    remoteAudio.set('peer',{stop:()=>{audio.srcObject=null;},setVolume:vol=>{audio.volume=vol/100;}});
   }else if(ev.track.kind==='video'&&type==='video'){
    const remoteTarget=scope==='group'?ensureGroupVideoTarget('peer'):$('video-remote');
    if(remoteTarget){remoteTarget.innerHTML='';const vid=document.createElement('video');vid.autoplay=true;vid.playsInline=true;vid.srcObject=stream;remoteTarget.appendChild(vid);}
    remoteVideo.set('peer',{stop:()=>{}});
   }
  };
  pc.oniceconnectionstatechange=()=>{
   if(pc.iceConnectionState==='connected'||pc.iceConnectionState==='completed'){onConnected();}
   else if(pc.iceConnectionState==='failed'||pc.iceConnectionState==='disconnected'){onFail(new Error(`${provider} WebRTC ${pc.iceConnectionState}`));}
  };
  if(call.ref){
   const isCaller=call.outgoing,myCol=isCaller?'callerCandidates':'receiverCandidates',theirCol=isCaller?'receiverCandidates':'callerCandidates';
   pc.onicecandidate=ev=>{if(ev.candidate&&call.ref)call.ref.collection(myCol).add(ev.candidate.toJSON()).catch(()=>{});};
   if(isCaller){
    const offer=await pc.createOffer();await pc.setLocalDescription(offer);
    await call.ref.update({offer:{type:offer.type,sdp:offer.sdp},provider}).catch(()=>{});
    call.signalingUnsub=call.ref.onSnapshot(async snap=>{
     const d=snap.data();if(d?.answer&&!pc.currentRemoteDescription)await pc.setRemoteDescription(new RTCSessionDescription(d.answer)).catch(()=>{});
    });
   }else{
    const snap=await call.ref.get(),d=snap.data();
    if(d?.offer){
     await pc.setRemoteDescription(new RTCSessionDescription(d.offer));
     const answer=await pc.createAnswer();await pc.setLocalDescription(answer);
     await call.ref.update({answer:{type:answer.type,sdp:answer.sdp,provider},status:'connected'}).catch(()=>{});
    }
   }
   call.ref.collection(theirCol).onSnapshot(snap=>{snap.docChanges().forEach(change=>{if(change.type==='added')pc.addIceCandidate(new RTCIceCandidate(change.doc.data())).catch(()=>{});});});
  }
...
async function cleanupProvider(provider){
 if(provider==='agora'){
  ...
  if(localAudio){localAudio.stop();localAudio.close();localAudio=null;}
 ...
 remoteAudio.clear();remoteVideo.clear();
}

async function joinTelephonyCall(scope,id,type,failedProviders=[]){
 const session=await fetchTelephonySession(scope,id,type,failedProviders);
 call.currentProvider=session.provider;
 call.currentRole=session.role;
 const timeoutMs=Number(session.failoverTimeoutMs||6000);
 return new Promise((resolve,reject)=>{
  let settled=false;
  const onConnected=()=>{
   if(settled)return;settled=true;clearTimeout(failoverTimer);
   if(scope==='group')status(type==='video'?'groupVideo':'groupAudio',type);
   else if(call.status==='connected'){status('connected',type);startTimer();}
   resolve(session);
  };
  const onFail=async(err)=>{
   if(settled||ending)return;settled=true;clearTimeout(failoverTimer);
   ...
   if(nextFailed.length<4){
    ...
    reject(err||new Error('ALL_TELEPHONY_PROVIDERS_EXHAUSTED'));
   }
  };
  failoverTimer=setTimeout(()=>{onFail(new Error(`Timeout connecting to ${session.provider}`));},timeoutMs);
...
async function joinAgora(scope,id,type){
 return await joinTelephonyCall(scope,id,type,[]);
}
...
async function startDirect(type){
 ...
 }catch(error){console.error('Telephony call start failed',error);alert(mediaError(error));await finish(true);}
}

async function accept(){
 if(!call.ref||call.outgoing)return;
 try{activeControls(call.type);status('connecting',call.type);await joinAgora('direct',call.id,call.type);call.status='connected';await call.ref.update({answer:{provider:call.currentProvider||'agora'},status:'connected'});startTimer();}
 catch(error){console.error('Telephony call accept failed',error);alert(mediaError(error));await call.ref.update({status:'rejected',endedAt:firebase.firestore.FieldValue.serverTimestamp()}).catch(()=>{});await finish(false);}
}
...
async function finish(update=true){
 ...
 const duration=previous.start?Math.max(0,Math.floor((Date.now()-previous.start)/1000)):0;
 if(duration>0&&previous.currentProvider&&window.accountFunctions){
  window.accountFunctions.httpsCallable('recordCallDuration')({provider:previous.currentProvider,durationSeconds:duration,callType:previous.type}).catch(()=>{});
 }
...
async function shareScreen(){
 ...
 try{const created=await window.AgoraRTC.createScreenVideoTrack({},'disable');screenVideo=Array.isArray(created)?created[0]:created;if(localVideo&&client)await client.unpublish(localVideo);if(client)await client.publish(screenVideo);screenVideo.play(ui.localVideo);screenVideo.on('track-ended',stopShare);label(ui.shareScreen,'stopShare','active');}catch(error){if(error?.code!=='PERMISSION_DENIED')console.warn('Screen share failed',error);}
}
...
window.startGroupCall=async(group,type='audio')=>{
 if(call.id||groupId)return;groupId=group.id;groupType=type;Object.assign(call,{scope:'group',groupId:group.id,type,status:'connected',failedProviders:[]});ending=false;
 try{
 ...
 }catch(error){console.error('Telephony group call failed',error);alert(mediaError(error));await leaveGroup();}
};
