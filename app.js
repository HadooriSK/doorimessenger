    ...
    function renderChatList() {
        ...
        const createItem = (chat) => {
            ...
            if (chat.type !== 'room' && chat.type !== 'channel' && chat.type !== 'saved' && chat.type !== 'assistant') {
                let p = getUserProfile(chat.name || chat.id);
                const rawStatus = p && (p.bio || p.status) ? (p.bio || p.status) : '';
                const presence = presenceByUser.get(String(chat.id).toLowerCase());
                const lastSeenMs = presence?.lastSeen?.toMillis?.() || (presence?.lastSeen?.seconds ? presence.lastSeen.seconds * 1000 : Number(presence?.lastSeen || 0));
                const isOnlineNow = !!presence?.isOnline && lastSeenMs > 0 && Date.now() - lastSeenMs <= 75000;
                if (isOnlineNow) presenceDotHtml = `<span class="contact-presence-dot ${rawStatus && rawStatus.toLowerCase() !== 'online' ? 'busy' : 'online'}" aria-hidden="true"></span>`;
                if (rawStatus && rawStatus.toLowerCase() !== 'online') {
                    const pKey = typeof getPresetKeyFromStatus === 'function' ? getPresetKeyFromStatus(rawStatus) : null;
                    const activeLang = window.currentLang || currentLang || 'en';
                    const t = TRANSLATIONS[activeLang] || TRANSLATIONS['en'] || {};
            ...
            div.innerHTML = safeHTML(`<div class="avatar ${chat.type === 'room' || chat.type === 'channel' ? 'room-avatar' : ''}" style="position:relative;">${avatarHtml}${mentionBadgeHtml}${presenceDotHtml}</div><div class="chat-item-info"><span class="chat-item-name">${escapeHTML(getTranslatedChatName(chat))}${statusBadgeHtml}</span></div>`);
        ...
        const presenceIds = new Set([...(cData.personal || []), ...(cData.contacts || []), ...(cData.active_chats || [])]
            .filter(c => !['room','channel','group','saved','assistant'].includes(c.type))
            .map(c => String(c.id).toLowerCase()));
        presenceIds.forEach(id => {
            if (presenceListeners.has(id) || !window.db) return;
            presenceListeners.set(id, window.db.collection('presence').doc(id).onSnapshot(doc => {
                if (doc.exists) presenceByUser.set(id, doc.data()); else presenceByUser.delete(id);
                renderChatList();
            }, () => presenceByUser.delete(id)));
        });
        presenceListeners.forEach((unsubscribe, id) => {
            if (!presenceIds.has(id)) { unsubscribe(); presenceListeners.delete(id); presenceByUser.delete(id); }
        });
        if (!presenceRefreshTimer) presenceRefreshTimer = setInterval(renderChatList, 30000);
