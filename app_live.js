    ...
    if (copyLinkBtn) {
        copyLinkBtn.addEventListener('click', () => {
            if (currentChat && currentChat.type === 'room') {
                const link = `${window.location.origin}${window.location.pathname}?join=${currentChat.id}`;
...
async function sendMessage(text, mediaType = null, mediaUrl = null, silent = false, scheduleTime = null) {
    ...
    function selectChat(id, type) {
        ...
        if (type === 'dm') {
            currentChatStatus.style.display = 'inline-block'; if (callBtn) callBtn.style.display = 'flex'; const doodleBtn = document.getElementById('doodle-btn'); if (doodleBtn) doodleBtn.style.display = 'inline-block'; const videoBtn = document.getElementById('video-call-btn'); if (videoBtn) videoBtn.style.display = 'flex'; const callContainer = document.getElementById('call-buttons-container'); if(callContainer) callContainer.style.display = 'flex';
    ...
    if (addMemberBtn) {
        addMemberBtn.addEventListener('click', () => {
            const username = prompt((window.TRANSLATIONS[window.currentLang] || {}).placeholder_search || "Benutzername eingeben:");
            if (username) {
                let u = username.trim().toLowerCase();
                if (!u.startsWith('@')) u = '@' + u;
                if (currentChat.members.includes(u)) return;
                
                window.db.collection('users').doc(u).get().then(doc => {
                    if(doc.exists) {
    ...
    window.showInviteModal = function(msg) {
        ...
        document.getElementById('accept-invite-btn').onclick = () => {
            window.acceptGroupInvite(msg.id, msg.invite_group_id);
    ...
    window.acceptGroupInvite = async function(groupId, msgId, sender, groupName) {
        try {
            // Delete all invites for this group for the current user
            const invitesQuery = await window.db.collection('messages')
                .where('type', '==', 'group_invite')
                .where('invite_group_id', '==', groupId)
                .where('recipient_username', '==', currentUser.toLowerCase())
                .get();
            const batch = window.db.batch();
            invitesQuery.forEach(doc => batch.delete(doc.ref));
            await batch.commit();
            ...
            if (doc.exists) {
                ...
                // Add to local rooms if not already there
                ...
                alert((window.TRANSLATIONS[window.currentLang] || window.TRANSLATIONS['en']).msg_group_deleted || 'Die Gruppe existiert nicht mehr.');
            }
        } catch(e) { console.error(e); }
    };
    ...
    window.declineGroupInvite = async function(groupId, msgId, sender, groupName) {
        try {
            ...
            const text = t.msg_declined_invite ? t.msg_declined_invite.replace('{group}', groupName) : `❌ Habe die Einladung zur Gruppe "${groupName}" abgelehnt.`;
            await msgRef.set({
                id: msgRef.id,
                chat_id: sender.toLowerCase(),
                sender_username: currentUser,
                recipient_username: sender.toLowerCase(),
                timestamp: Date.now(),
                text: text,
                isPublic: false,
                participants: [currentUser.toLowerCase(), sender.toLowerCase()],
                type: 'text'
            });
        } catch(e) { console.error(e); }
    };
    ...
    window.acceptGroupInvite = async function(msgId, groupId) {
        // Removed duplicate to prevent overwriting the fully featured function above
    };
    ...
    const phase2Additions = {
        de: { lbl_blocked_contacts: 'Blockierte Kontakte', btn_unblock: 'Entblocken', lbl_enter_to_send: 'Mit Enter senden', btn_export_chat: 'Chat exportieren', msg_no_blocked: 'Keine blockierten Kontakte.', msg_chat_exported: 'Chat erfolgreich exportiert.', btn_clear_chat: 'Chat leeren', msg_confirm_clear: 'Diesen Chat wirklich für alle leeren?', msg_confirm_block: 'Sind Sie sich sicher, dass Sie den Benutzer blockieren möchten? (Ja / Nein)', ctx_add_contact: 'Zu Kontakten hinzufügen', ctx_mute_user: 'Stummschalten', ctx_unmute_user: 'Stummschaltung aufheben', ctx_clear_chat: 'Chat leeren', ctx_block_user: 'Blockieren', ctx_unblock_user: 'Entblocken', msg_declined_invite: '❌ Habe die Einladung zur Gruppe "{group}" abgelehnt.', btn_edit_group: 'Gruppe bearbeiten', btn_delete_group: 'Gruppe löschen', lbl_choose_from_contacts: 'Aus Kontakten wählen:', lbl_or_search_user: 'Oder Benutzer suchen:', ph_group_id: 'Gruppen-ID (6-stellig)', err_group_id_invalid: 'Die Gruppen-ID muss aus genau 6 Zahlen bestehen.', err_group_id_taken: 'Diese Gruppen-ID ist bereits vergeben. Bitte wähle eine andere.' },
        en: { lbl_blocked_contacts: 'Blocked Contacts', btn_unblock: 'Unblock', lbl_enter_to_send: 'Send with Enter', btn_export_chat: 'Export Chat', msg_no_blocked: 'No blocked contacts.', msg_chat_exported: 'Chat exported successfully.', btn_clear_chat: 'Clear Chat', msg_confirm_clear: 'Really clear this chat for everyone?', msg_confirm_block: 'Are you sure you want to block this user? (Yes / No)', ctx_add_contact: 'Add to Contacts', ctx_mute_user: 'Mute', ctx_unmute_user: 'Unmute', ctx_clear_chat: 'Clear Chat', ctx_block_user: 'Block', ctx_unblock_user: 'Unblock', msg_declined_invite: '❌ Declined the invitation to group "{group}".', btn_edit_group: 'Edit Group', btn_delete_group: 'Delete Group', lbl_choose_from_contacts: 'Choose from contacts:', lbl_or_search_user: 'Or search user:', ph_group_id: 'Group ID (6 digits)', err_group_id_invalid: 'The Group ID must consist of exactly 6 digits.', err_group_id_taken: 'This Group ID is already taken. Please choose another.' },
        fa: { lbl_blocked_contacts: 'مخاطبین مسدود شده', btn_unblock: 'رفع مسدودیت', lbl_enter_to_send: 'ارسال با اینتر', btn_export_chat: 'خروجی چت', msg_no_blocked: 'هیچ مخاطب مسدود شده ای وجود ندارد.', msg_chat_exported: 'چت با موفقیت صادر شد.', btn_clear_chat: 'پاک کردن چت', msg_confirm_clear: 'آیا این چت برای همه پاک شود؟', msg_confirm_block: 'آیا مطمئن هستید که می‌خواهید این کاربر را مسدود کنید؟ (بله / خیر)', ctx_add_contact: 'افزودن به مخاطبین', ctx_mute_user: 'بی‌صدا کردن', ctx_unmute_user: 'صدادار کردن', ctx_clear_chat: 'پاک کردن چت', ctx_block_user: 'مسدود کردن', ctx_unblock_user: 'رفع مسدودیت', msg_declined_invite: '❌ دعوت به گروه "{group}" را رد کردم.', btn_edit_group: 'ویرایش گروه', btn_delete_group: 'حذف گروه', lbl_choose_from_contacts: 'انتخاب از مخاطبین:', lbl_or_search_user: 'یا جستجوی کاربر:', ph_group_id: 'شناسه گروه (۶ رقم)', err_group_id_invalid: 'شناسه گروه باید دقیقاً ۶ رقم باشد.', err_group_id_taken: 'این شناسه گروه قبلاً گرفته شده است. لطفاً شناسه دیگری انتخاب کنید.' },
        ar: { lbl_blocked_contacts: 'جهات الاتصال المحظورة', btn_unblock: 'إلغاء الحظر', lbl_enter_to_send: 'إرسال بواسطة إدخال', btn_export_chat: 'تصدير الدردشة', msg_no_blocked: 'لا توجد جهات اتصال محظورة.', msg_chat_exported: 'تم تصدير الدردشة بنجاح.', btn_clear_chat: 'مسح الدردشة', msg_confirm_clear: 'هل تريد حقًا مسح هذه الدردشة للجميع؟', msg_confirm_block: 'هل أنت متأكد أنك تريد حظر هذا المستخدم؟ (نعم / لا)', ctx_add_contact: 'إضافة إلى جهات الاتصال', ctx_mute_user: 'كتم الصوت', ctx_unmute_user: 'إلغاء كتم الصوت', ctx_clear_chat: 'مسح الدردشة', ctx_block_user: 'حظر', ctx_unblock_user: 'إلغاء الحظر', msg_declined_invite: '❌ رفضت الدعوة للمجموعة "{group}".', btn_edit_group: 'تعديل المجموعة', btn_delete_group: 'حذف المجموعة', lbl_choose_from_contacts: 'اختر من جهات الاتصال:', lbl_or_search_user: 'أو ابحث عن مستخدم:', ph_group_id: 'معرف المجموعة (6 أرقام)', err_group_id_invalid: 'يجب أن يتكون معرف المجموعة من 6 أرقام بالضبط.', err_group_id_taken: 'معرف المجموعة هذا مأخوذ بالفعل. يرجى اختيار واحد آخر.' },
        tr: { lbl_blocked_contacts: 'Engellenen Kişiler', btn_unblock: 'Engeli Kaldır', lbl_enter_to_send: 'Enter ile Gönder', btn_export_chat: 'Sohbeti Dışa Aktar', msg_no_blocked: 'Engellenen kişi yok.', msg_chat_exported: 'Sohbet başarıyla dışa aktarıldı.', btn_clear_chat: 'Sohbeti Temizle', msg_confirm_clear: 'Bu sohbeti herkes için gerçekten temizle?', msg_confirm_block: 'Bu kullanıcıyı engellemek istediğinizden emin misiniz? (Evet / Hayır)', ctx_add_contact: 'Kişilere Ekle', ctx_mute_user: 'Sessize Al', ctx_unmute_user: 'Sesi Aç', ctx_clear_chat: 'Sohbeti Temizle', ctx_block_user: 'Engelle', ctx_unblock_user: 'Engeli Kaldır', msg_declined_invite: '❌ "{group}" grubuna daveti reddettim.', btn_edit_group: 'Grubu Düzenle', btn_delete_group: 'Grubu Sil', lbl_choose_from_contacts: 'Kişilerden seç:', lbl_or_search_user: 'Veya kullanıcı ara:', ph_group_id: 'Grup Kimliği (6 haneli)', err_group_id_invalid: 'Grup Kimliği tam olarak 6 rakamdan oluşmalıdır.', err_group_id_taken: 'Bu Grup Kimliği zaten alınmış. Lütfen başka bir tane seçin.' }
    };
