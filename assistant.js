(function(root){
 ...
 const MEMORY_TEXT={
  de:{title:'Doori-Erinnerung',description:'Doori merkt sich wichtige Gesprächsinhalte sparsam für Text- und Live-Gespräche.',clear:'Alle Erinnerungen löschen',confirm:'Möchtest du wirklich alle Doori-Erinnerungen löschen?',done:'Alle Doori-Erinnerungen wurden gelöscht.'},
  en:{title:'Doori memory',description:'Doori efficiently remembers important conversation details for text and Live conversations.',clear:'Delete all memories',confirm:'Do you really want to delete all Doori memories?',done:'All Doori memories have been deleted.'},
  ar:{title:'ذاكرة Doori',description:'يتذكر Doori تفاصيل المحادثة المهمة بكفاءة للمحادثات النصية والمباشرة.',clear:'حذف جميع الذكريات',confirm:'هل تريد حقًا حذف جميع ذكريات Doori؟',done:'تم حذف جميع ذكريات Doori.'},
  fa:{title:'حافظه Doori',description:'Doori نکات مهم گفت‌وگوهای متنی و زنده را به‌صورت کم‌حجم به خاطر می‌سپارد.',clear:'حذف همه خاطره‌ها',confirm:'آیا واقعاً می‌خواهید همه خاطره‌های Doori حذف شوند؟',done:'همه خاطره‌های Doori حذف شدند.'},
  tr:{title:'Doori hafızası',description:'Doori, metin ve Canlı konuşmalardaki önemli ayrıntıları verimli biçimde hatırlar.',clear:'Tüm anıları sil',confirm:'Tüm Doori anılarını gerçekten silmek istiyor musun?',done:'Tüm Doori anıları silindi.'}
 };
 ...
 function updateMemorySettings(){const copy=MEMORY_TEXT[lang()]||MEMORY_TEXT.en,title=document.getElementById('assistant-memory-title'),description=document.getElementById('assistant-memory-description'),button=document.getElementById('btn-clear-assistant-memory');if(title)title.textContent=copy.title;if(description)description.textContent=copy.description;if(button)button.textContent=copy.clear;}
 async function clearMemory(){const copy=MEMORY_TEXT[lang()]||MEMORY_TEXT.en;if(!root.confirm(copy.confirm))return;try{await root.accountFunctions.httpsCallable('clearDooriMemory')({});localStorage.removeItem(historyKey());sync();root.alert(copy.done);}catch(error){console.error('Could not clear Doori memory',error);}}
