(function(root){
 ...
 async function startListening(){if(state.starting||state.transcribing||state.busy)return;state.starting=true;updateControls();setStatus(t().starting);if(state.recorder?.state!=='recording'){state.voice=true;localStorage.setItem('doori_ai_voice','on');root.DooriTTS?.unlock?.();}root.DooriTTS?.stop();try{if(!await startRecording())browserRecognition();}finally{state.starting=false;updateControls();}}
