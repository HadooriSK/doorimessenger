(function(root){
 ...
 function updateControls(){
  ...
  if(mic){const recording=state.recorder?.state==='recording';mic.classList.toggle('hidden',!isActive());mic.textContent=recording?'⏹':'🎙️';mic.title=recording?t().stop:t().mic;mic.setAttribute('aria-label',mic.title);mic.setAttribute('aria-pressed',recording?'true':'false');}
 ...
 async function startRecording(){
  ...
  try{
   ...
   recorder.start(250);document.body.classList.add('assistant-recording');document.getElementById('assistant-mic-btn')?.classList.add('recording');updateControls();setStatus(t().listening);state.recordTimer=setTimeout(()=>{if(state.recorder===recorder&&recorder.state==='recording')recorder.stop();},30000);return true;
