...
async function callGroq({fetchImpl,key,messages,language,signal}){
 if(!key)return null;
  const response=await fetchImpl(GROQ_URL,{method:'POST',headers:{Authorization:`Bearer ${key}`,'Content-Type':'application/json'},body:JSON.stringify({model:GROQ_MODEL,temperature:0.55,max_completion_tokens:360,messages:[{role:'system',content:systemPrompt(language)},...messages]}),signal});
...
async function callGemini({fetchImpl,key,messages,language,signal}){
 ...
 const response=await fetchImpl(`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent`,{method:'POST',headers:{'x-goog-api-key':key,'Content-Type':'application/json'},body:JSON.stringify({systemInstruction:{parts:[{text:systemPrompt(language)}]},contents,generationConfig:{temperature:0.55,maxOutputTokens:240}}),signal});
 if(!response.ok)throw Object.assign(new Error('PROVIDER_HTTP'),{status:response.status});
