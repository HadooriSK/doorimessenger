...
const assistantDefaults={textMessagesPerUser:100,voiceSecondsPerUser:3600,speechRecognitionSecondsPerDay:28000,geminiSpeechDailyRequests:100,geminiTtsDailyRequests:100,groqDailyRequests:200,geminiDailyRequests:200,cloudflareDailyNeurons:10000,providerTimeoutMs:8000};
...
function validSpeechResult(result,fallback){const text=String(result?.text||'').trim(),language=speechLanguage(result?.language,text,fallback),declared=String(result?.language||'').toLowerCase(),allowed=/^(de|german|deutsch|en|english|tr|turkish|ar|arabic|fa|persian|farsi)$/;const hasUnsupportedScript=/[\p{Script=Han}\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Hangul}\p{Script=Cyrillic}]/u.test(text);if(!text||hasUnsupportedScript)throw Object.assign(new Error('Unsupported transcription language'),{status:422});if(declared&&!allowed.test(declared))throw Object.assign(new Error('Unsupported transcription language'),{status:422});return {text,language};}
