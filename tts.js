(function(root){
 ...
 function speak(value,{language}={}){const expected=shortLanguage(language),text=cleanPunctuationWords(value,expected),url=endpoint();if(!/[\p{L}\p{N}]/u.test(text))return false;if(!url)return systemSpeak(text,expected);return neuralSpeak(text,expected,url).catch(error=>{console.warn('Doori neural TTS unavailable; using system fallback.',error?.message||'error');root.dispatchEvent(new CustomEvent('doori-tts-fallback',{detail:{language:expected,model:engineFor(expected)}}));return systemSpeak(text,expected);});}
