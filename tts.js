(function(root){
 ...
 async function geminiSpeak(text,language){if(!root.accountFunctions?.httpsCallable)throw new Error('TTS_UNAVAILABLE');const result=await root.accountFunctions.httpsCallable('synthesizeDooriSpeech')({text,language,gender}),data=result?.data||{};if(!data.audioBase64)throw new Error('TTS_AUDIO');stop();const url=URL.createObjectURL(base64Audio(data.audioBase64,data.mimeType));activeAudio=new Audio(url);activeAudio.onended=()=>{URL.revokeObjectURL(url);activeAudio=null;};activeAudio.onerror=()=>{URL.revokeObjectURL(url);activeAudio=null;};await activeAudio.play();return true;}
