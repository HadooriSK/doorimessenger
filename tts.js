(function(root){
 ...
 async function unlock(){try{const audio=player();if(audioUnlocked)return true;const url=URL.createObjectURL(silentWav());audio.src=url;audio.muted=true;await audio.play();audio.pause();audio.currentTime=0;audio.muted=false;URL.revokeObjectURL(url);audioUnlocked=true;return true;}catch{return false;}}
